...
class SignUp(Resource):
    ...
    def post(self):
        ...
        if user.email is not None:
            msg=Message("registration user account",sender="yosser.riahi123@gmail.com",recipients=[f"{user.email}"])
            msg.body=f"Welcome to OnePeace Patient ,you have created an account with succes, please verify your account before login with code identif {user.code}"
            mail.send(msg)
            user.save()
            response={
